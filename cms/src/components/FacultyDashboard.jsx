...
const FacultyDashboard = ({ selectedMonth }) => {
  ...
  useEffect(() => {
    ...
    const facultySubjects = JSON.parse(localStorage.getItem("facultySubjects")) || {};
  ...
  const fetchAttendancePercentage = async (month) => {
    ...
    try {
      const response = await axios.post("http://localhost:5472/services/getstafdashcount", {
        ...
        staff_id: parseInt(sessionStorage.getItem("staff_id")),
      });
  ...
  useEffect(() => {
    const staffId = parseInt(sessionStorage.getItem("staff_id"));

    const fetchSubjectsByStaffId = async () => {
      try {
        // Make an API call to get subjects by staff ID
        ...
        const subjectsData = response.data.map((item) => item.subject) || []; // Extract the subjects
        setSubjects(subjectsData); // Set the subjects into the state

        // If the faculty has subjects, set the first subject as the default selected one
