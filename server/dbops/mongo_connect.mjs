...
app.post('/services/getstafdashcount', async (req, res) => {
  ...
  try {
    ...
    if (attendanceCount === 0) {
      res.status(200).send({ absent: 0, present: 0, students: studentCount });
    } else {
      res.status(200).send({ absent: absentPercentage, present: presentPercentage, students: studentCount,staffname: staffName["Staff_name"],Class: staffName["Assigned_Class"]});
    }
  } catch (error) {
    console.error('Error deleting record:', error);
    res.status(500).send({ message: 'Server error' });
...
const updatestaffdoc = async (stdid, picurl) => {
  ...
  try {
    ...
    if (updatepic.matchedCount === 0) {
      ...
      return true; // The document was found but the update may not have changed anything
    } else {
